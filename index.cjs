...
function getPlatformKey() {
  ...
  throw new Error(
    `Unsupported node-obscura platform: ${process.platform}/${process.arch}.`
  );
}

function getBinaryPath() {
  const binaryPath = path.join(__dirname, 'binaries', getPlatformKey(), 'obscura');
  if (!fs.existsSync(binaryPath)) {
    throw new Error(
      `Obscura binary not found at ${binaryPath}. Reinstall node-obscura so postinstall can fetch it.`
    );
  }
  return binaryPath;
}
